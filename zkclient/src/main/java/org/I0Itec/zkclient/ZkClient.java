...
public class ZkClient implements Watcher {
   ...
   * @throws RuntimeException         if any other exception occurs
   */
  public void create(final String path, Object data, final CreateMode mode, final AsyncCallback.StringCallback callback, final Object context) throws ZkInterruptedException, IllegalArgumentException, ZkException, RuntimeException {
    if (path == null) {
      throw new NullPointerException("path must not be null.");
    }
    final byte[] bytes = data == null ? null : serialize(data);
    ...
    retryUntilConnected(new Callable<Boolean>() {
      ...
      public Boolean call() throws Exception {
        _connection.create(path, bytes, mode, callback, context);
        return Boolean.TRUE;
      }
    });
  }
  ...
  public void delete(final String path, final AsyncCallback.VoidCallback callback, final Object context) {
    try {
      retryUntilConnected(new Callable<Boolean>() {
        ...
        @Override
        public Boolean call() throws Exception {
          _connection.delete(path, callback, context);
          return Boolean.TRUE;
        }
      });
